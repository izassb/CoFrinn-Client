...
function Perfil() {
  ...
  useEffect(()=>{
    ...
    AuthService.getById(user.id).then(response =>{
      ...
      setValues([{senha: response.data[0].senha, showPassword: !values.showPassword }]);
    })
  },[])
      ...
      <div className="container">
        ...
        <Form.Group className="mb-3" controlId="formBasicEmail">
          ...
          <div className="buttons">
            <Button variant="second" size="lg" onClick={handleSubmit}> Salvar </Button>
            <Button variant="primary" size="lg" onClick={()=>{window.location.href="/home"}}> Cancelar </Button>
          </div>
        </Form>
        <div className="description2">
         <Button variant="primary" size="lg" onClick={()=>{AuthService.cleanLoggedUser()}}> Sair da conta </Button>
        </div>
      </div>
    </div>
  );
