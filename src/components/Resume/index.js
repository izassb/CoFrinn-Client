...
const Resume = () => {
  ...
  useEffect(() => {
    AuthService.obterSaldoUsuario(user.id)
      .then((response) => {
    ...
    AuthService.obterReceitaUsuario(user.id)
      .then((response) => {
    ...
    AuthService.obterDespesaUsuario(user.id)
      .then((response) => {
  ...
  return (
    <C.Container>
      ...
      <ResumeItem
        ...
        value={totalDespesa}
      />
      <ResumeItem title="Total" Icon={FaDollarSign} value={totalSaldo} />
    </C.Container>
  );
};
